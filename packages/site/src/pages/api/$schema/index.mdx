...
With the `$schema` constructor you can create schemas very easily and most importantly, these schemas can be reused throughout the application, especially in the `params` option of <a href="/api/$route#params">$route</a>, <a href="/api/$settings#params">$settings</a>, <a href="/api/$session#params">$session</a> and <a href="/api/$api#params">$api</a>.
